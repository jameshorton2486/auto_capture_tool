...
class AutoCaptureTool:
    # Maximum zip file size before splitting (29MB)
    MAX_ZIP_SIZE_BYTES = 29 * 1024 * 1024
    ...
    def __init__(self, root):
        ...
        self.chrome_user_data_dir = os.path.join(
            os.path.expanduser("~"), 
            ".auto_capture_tool", 
            "chrome_profile"
        )
    ...
    def _build_ui(self):
        ...
        # Default save directory - use user's Documents folder as fallback
        default_dir = r"G:\My Drive\Kollect-It\CTBids Screen Shots"
        fallback_dir = os.path.join(os.path.expanduser("~"), "Documents", "Screenshots")
        ...
        try:
            if os.path.exists(os.path.dirname(default_dir)):
                ...
                raise FileNotFoundError("Default path not available")
        except Exception:
            # Use fallback directory
            try:
                os.makedirs(fallback_dir, exist_ok=True)
                self.entry_dir.delete(0, tk.END)
                self.entry_dir.insert(0, fallback_dir)
                self.root_save_directory = fallback_dir
            except Exception:
                pass
        ...
        session_chk = tk.Checkbutton(
            opt_frame,
            text="Keep login sessions",
        ...
        session_chk.pack(side=tk.LEFT, padx=(20, 0))
        ...
        # Login button for manual authentication
    ...
    def open_browser_for_login(self):
        """Open browser so user can login manually before capturing."""
        if self.driver:
            messagebox.showinfo("Browser Open", "Browser is already open. Login and then click Start.")
            return
        ...
        try:
            ...
            if self.persist_session_var.get():
                ...
                self.log("Using persistent Chrome profile for session storage")
            ...
            # Navigate to first URL if available, otherwise show blank
            urls = self.extract_urls(self.text_area.get("1.0", tk.END))
            if urls:
                self.driver.get(urls[0])
                self.log(f"Navigated to {urls[0]} - Login if needed, then click Start")
            else:
                self.log("Browser opened. Paste URLs, login if needed, then click Start")
                
            messagebox.showinfo(
                "Login Ready", 
                "Browser opened.\n\n"
                "1. Login to any sites that require authentication\n"
                "2. Click Start when ready to capture"
            )
    ...
    def _capture_loop(self):
        browser_was_open = self.driver is not None
        
        try:
            width = int(self.width_var.get())
            delay = int(self.delay_var.get())

            # Only create new browser if one isn't already open
            if not self.driver:
                options = Options()
            ...
            if self.is_running:
                ...
                self.root.after(0, lambda: messagebox.showinfo("Done", "Capture completed."))

        except Exception as e:
            self.log(f"Fatal error: {e}")

        finally:
            # Only quit browser if we created it during capture
            if self.driver and not browser_was_open:
                self.driver.quit()
                self.driver = None
    ...
    def _save_file(self, item, screenshot_bytes):
        ...
        if fmt == "png":
            ...
            img = AutoCaptureTool.flatten_rgba(img)  # Fixed: use class name for static method
    ...
    # ==================================================
    def zip_files(self):
        """Create zip file(s) of all captured screenshots."""
        folder = self.entry_dir.get().strip()
        if not folder or not os.path.exists(folder):
            messagebox.showerror("Error", "Save folder doesn't exist.")
            return
            
        if self.is_zipping:
            messagebox.showinfo("Info", "Zip operation already in progress.")
            return
        
        # Get list of files to zip (excluding existing zip files)
        files_to_zip = []
        for root_dir, dirs, files in os.walk(folder):
            for file in files:
                if not file.lower().endswith('.zip'):
                    files_to_zip.append(os.path.join(root_dir, file))
        
        if not files_to_zip:
            messagebox.showinfo("Info", "No files to zip in the save directory.")
            return
        
        # Ask for save location
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_name = f"screenshots_{timestamp}.zip"
        ...
        self.log(f"Starting zip operation with {len(files_to_zip)} files...")
        
        # Run in background thread
        threading.Thread(
            target=self._create_zip_files,
            args=(folder, files_to_zip, save_path),
            daemon=True
        ).start()

    def _create_zip_files(self, source_folder, files_to_zip, save_path):
        """Create zip file(s), splitting if necessary."""
        try:
            base_path = save_path.rsplit('.zip', 1)[0]
            created_zips = []
            current_zip_num = 1
            current_zip_size = 0
            current_zip_path = save_path
            current_zip = None
            ...
            for i, filepath in enumerate(files_to_zip):
                ...
                if current_zip is not None:
                    # Estimate size after adding this file
                    if current_zip_size + file_size > self.MAX_ZIP_SIZE_BYTES:
                        # Close current zip and start a new one
                        current_zip.close()
                        created_zips.append((current_zip_path, current_zip_size))
                        self.log(f"Created {os.path.basename(current_zip_path)} ({self._format_size(current_zip_size)})")
                        
                        current_zip_num += 1
                        current_zip_path = f"{base_path}_part{current_zip_num}.zip"
                        current_zip = zipfile.ZipFile(current_zip_path, 'w', zipfile.ZIP_DEFLATED)
                        current_zip_size = 0
                
                # Create first zip if needed
                if current_zip is None:
                    if len(files_to_zip) > 1:
                        # Check if we might need multiple parts
                        total_size = sum(os.path.getsize(f) for f in files_to_zip)
                        if total_size > self.MAX_ZIP_SIZE_BYTES:
                            current_zip_path = f"{base_path}_part1.zip"
                    current_zip = zipfile.ZipFile(current_zip_path, 'w', zipfile.ZIP_DEFLATED)
                ...
                # Update progress
                progress_msg = f"Zipping: {i+1}/{total_files} files"
                self.root.after(0, lambda m=progress_msg: self.progress_var.set(m))
            ...
            if current_zip:
                ...
                created_zips.append((current_zip_path, actual_size))
                self.log(f"Created {os.path.basename(current_zip_path)} ({self._format_size(actual_size)})")
            
            # Show completion message
            summary = f"Zip complete!\n\nCreated {len(created_zips)} file(s):\n"
            for zip_path, size in created_zips:
                summary += f"\n• {os.path.basename(zip_path)} ({self._format_size(size)})"
            
            self.log(f"Zip operation complete. Created {len(created_zips)} file(s).")
            self.root.after(0, lambda: messagebox.showinfo("Zip Complete", summary))
